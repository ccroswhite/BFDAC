...
module artix_dac_top (
    ...
    always_ff @(posedge dsp_clk) begin
        if (!sys_rst_n) begin
            ...
            // Pulse exactly once per 1024-cycle BRAM read window. Keeping the
            // strobe non-constant prevents downstream synthesis from proving
            // master_coef_addr is stuck at zero, which would let Vivado fold
            // every polyphase MAC's multiplier output to a static value and
            // optimize all 256 DSPs out of the design.
            dsp_audio_valid_strobe <= (dsp_cache_raddr == 10'd0);
        end
    end
    ...
    fir_polyphase_interpolator #(
        ...
        .new_sample_data    (dsp_cache_rdata[31:8]),
