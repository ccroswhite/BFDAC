...
module polyphase_mac_engine #(
    ...
    // =================================---------------------------------------
    // Tag the heavy math to ensure it maps to the single DSP slice
    ...
    // Explicitly ban Vivado from using a 2nd DSP for the cascade addition.
    // This pushes the addition into ~48 standard LUTs per MAC, saving 256 DSPs.
    (* use_dsp = "no" *) logic signed [ACC_WIDTH-1:0] cascade_adder_fabric;
    assign cascade_adder_fabric = local_acc + acc_in;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (phase_sync) begin
                // Dump the cascade addition (computed safely in LUTs)
                acc_out   <= cascade_adder_fabric;
